...
- Before a release build, create `docs/releases/vX.Y.Z-N.md` from `docs/releases/TEMPLATE.md`. Use the matching version title, Chinese bullets followed by English bullets, and a full comparison link from the previous published stable release (excluding drafts and prereleases). Describe shipped user-facing additions, improvements, fixes, and relevant upgrade requirements; omit internal planning, version-only commits, placeholders, and workflow-run boilerplate. Validate it with `scripts/check-release-notes.py` as documented in `scripts/README.md`; the workflow uses this same tracked file for the release description.
