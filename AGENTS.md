...
Before any response or action, use the relevant available `superpowers` skill. If an optional local skill is unavailable, perform equivalent inspection, planning, and verification using these tracked requirements.

## Workflow

- The current tracked `AGENTS.md`, scripts, and workflow documentation define shared project conventions on every computer. Machine-local preferences and chat memory do not override explicit project conventions; the current user's explicit instructions still take precedence. Record durable project preferences in the repository in the same change.
- At the start of each task, fetch remote branches (including `main` and active task branches) and Git tags, inspect `git status` and local/remote divergence, and read the current shared instructions before choosing the task worktree. Preserve unfinished branches and unrelated changes; never reset or discard them to synchronize. Push the task branch before continuing it on another computer.
...
- Trigger remote GitHub DMG builds only from `main`. Merge and push the intended changes, updated `VERSION`, and matching release notes to `main` before dispatching `build-dmg.yml` with `--ref main`; the workflow rejects other branches and creates a draft by default.
- When explicitly asked to publish an official release, verify the successful `main` DMG build, its final assets, and validated release notes, then publish with `draft=false`, `prerelease=false`, and mark it as latest. Publishing an already verified draft does not require rebuilding or incrementing `VERSION`.
- Before a release build, create `docs/releases/vX.Y.Z-N.md` from `docs/releases/TEMPLATE.md`. Write release notes only in English, with the exact `# Inklet X.Y.Z (N)` title, a `## Changes` section with English bullets, and a `**Full changelog**` comparison link from the previous published stable release (excluding drafts and prereleases). Describe shipped user-facing additions, improvements, fixes, and relevant upgrade requirements; omit internal planning, version-only commits, placeholders, and workflow-run boilerplate. Validate it with `scripts/check-release-notes.py` as documented in `scripts/README.md`; the workflow uses this same tracked file for the release description.
...
- Before choosing the next build number, fetch remote branches (including `main` and active task branches) and Git tags, inspect all GitHub releases including drafts and prereleases, and coordinate with active linked worktrees and work on other computers. Make planned `VERSION` changes visible in pushed task branches before overlapping builds; local worktrees alone cannot reveal another computer's work. Choose a number greater than every already used or reserved build number and recheck before building or releasing; a stale worktree's `VERSION` is not sufficient. Inklet compares build numbers alone when checking for updates.
