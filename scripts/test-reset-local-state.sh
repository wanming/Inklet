...
if ! grep -q 'security delete-generic-password -s Inklet.Local.ProviderAPIKey' <<<"$reset_output"; then
  echo "reset-local-state.sh must delete local Keychain API keys." >&2
  exit 1
fi

if ! grep -q 'defaults delete com.tomwan.inklet.local' <<<"$reset_output"; then
  echo "reset-local-state.sh must delete local app defaults." >&2
  exit 1
fi

if ! grep -q 'tccutil reset Accessibility com.tomwan.inklet.local' <<<"$reset_output"; then
  echo "reset-local-state.sh must reset local app Accessibility permission." >&2
  exit 1
fi
